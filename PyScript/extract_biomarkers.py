...
def get_next_json_number():
    """Get the next available number for biomarkers JSON file."""
    existing_files = glob.glob("biomarkers*.json")
    ...
    for file in existing_files:
        try:
            # Extract number from filename (biomarkers1.json -> 1)
            num = int(file.replace("biomarkers", "").replace(".json", ""))
            numbers.append(num)
        except ValueError:
            continue
...
def save_biomarkers_json(data, file_number=None):
    """Save biomarkers data to JSON file."""
    try:
        # Determine filename
        if file_number is None:
            file_number = get_next_json_number()
        filename = f"biomarkers{file_number}.json"
        ...
        print(f"✅ Successfully saved biomarkers data to {filename}")
        return True
    except Exception as e:
        print(f"Error saving JSON file: {str(e)}")
        return False

def process_pdf_to_json(pdf_path, file_number=None):
    ...
    try:
        ...
        # Save to JSON file
        if save_biomarkers_json(json_data, file_number):
